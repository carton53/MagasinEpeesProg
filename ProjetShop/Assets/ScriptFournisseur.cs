...
public class ScriptFournisseur : MonoBehaviour
{
    ...
    public void OnEnable()
    {
        ...
        Debug.Log("j'achete à toute bersingue");
        Acheter();
    }
       ...
       foreach (Stock stock in magasin.Epees)
       {
           if (stock.nom ==nomEpees)
           {
                ...
                if(magasin.argent >= nombreFournisseur * prixFournisseur)
                {
                    ...
                    Debug.Log("J'ai acheté" + nombreFournisseur + " " + nomEpees);
                }
                else
                {
                    Debug.LogError("tu n'a pas la moula nécessaire pour te fournir en arme non redondante");
                }
